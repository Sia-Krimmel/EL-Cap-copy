import IMAGE_EXTENSIONS from '@modules/image-extensions';
...
export async function makeRequest({ endpoint, host, revalidate }: RequestProps) {
  try {
    const res = await fetch(`http://${host}/api/${endpoint}`, { next: { revalidate: revalidate ? revalidate : 0 } });
